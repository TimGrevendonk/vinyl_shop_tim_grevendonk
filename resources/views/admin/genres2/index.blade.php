...
@section('script_after')
    <script>
        // Execute the function  loadTable() as soon as the page loads
        loadTable();
        // Popup a dialog
        ...
        $('#modal-genre form').submit(function (e) {
            ...
            $.post(action, pars, 'json')
                ...
                // if an error, show error toast but do not reload page or close toast.
                .fail(function (e) {
                    console.log('error', e);
        ...
        function deleteGenre(id) {
            ...
            $.post(`/admin/genres2/${id}`, pars, 'json')
                .done(function (data) {
                    ...
                    // Show toast, function is in js/vinylShop.js
                    VinylShop.toast({
                        type: data.type,    // optional because the default type is 'success'
                        text: data.text,
                    });
                    // Old way, moved to vinylShop.js for general use.
                    /*new Noty({
                        type: data.type,
                        text: data.text,
        ...
        function loadTable() {
            $.getJSON('/admin/genres2/queryGenres')
                .done(function (data) {
                    console.log('data', data);
                    // Clear tbody tag
                    $('tbody').empty();
